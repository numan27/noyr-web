"use server";
import { privateRoutes, publicRoutes } from "./authRoutes";
import { routeConstant } from "./constants";

const getAuthentication = (isLoggedIn: boolean, pathName: string) => {
  switch (isLoggedIn) {
    case true:
      return {
        path: routeConstant.home.path,
        isAutherized: validatePath(privateRoutes, pathName),
      };
